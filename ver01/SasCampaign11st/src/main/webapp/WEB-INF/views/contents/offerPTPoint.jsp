...
<script type="text/javascript">
	// execute first on this jsp
	window.resizeTo(1060,450);
	/* ready */
	$(document).ready(function() {
		if (true) console.log("/contents/offerPTPoint.jsp: document.ready();");
		//document.body.scrollIntoView(true);
		//parent.document.all.offerAddFrame.height = document.body.scrollHeight;
		opener.fn_setIFrameHeight('offer');
	});
	...
	function fn_validation() {
		if ("${CAMP_STATUS_CD}" == "START") {
			...
			alert("진행중인 캠페인은 수정할수 없습니다.");
			return false;
		} else if ($("#DISP_NAME").val() == "") {
			alert("노출명을 입력하세요");
			$("#DISP_NAME").focus();
			return false;
		} else if ($("#OFFER_AMT").val() == "") {
			if ("${OFFER_TYPE_CD}" == "PN" || "${OFFER_TYPE_CD}" == "PT" || "${OFFER_TYPE_CD}" == "OC" ) {
				alert("포인트를 입력하세요");
				$("#OFFER_AMT").focus();
		...
		<div class="col-lg-10">
			...
			<form name="form" id="form">
				...
				<div class="col-lg-12" id="table">
					<table class="table table-striped table-hover table-condensed table-bordered" style="width:100%; border-spacing:0px; border-collapse:collapse;">
						<colgroup>
							<col width="130"/>
							<col width="250"/>
							<col width="130"/>
